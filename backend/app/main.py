...
# Downloads yolov8n-seg.pt (~6 MB) if not already cached
...
def sanitize_and_save(image_bytes, out_path):
    ...
    - Detects all persons using YOLOv8 segmentation and blurs them fully
    """
    ...
    for r in results:
        ...
        for mask, cls_id in zip(r.masks.data, r.boxes.cls):
            ...
            if label == "person":
                # Convert mask (float tensor) to numpy bool mask
                mask_np = mask.cpu().numpy().astype(bool)
                if mask_np.shape != img.shape[:2]:
                    continue
                # Create blurred version of full image
                blurred = cv2.GaussianBlur(img, (51, 51), 0)
                # Apply mask: copy blurred pixels onto original
                img[mask_np] = blurred[mask_np]

    # Save sanitized image (no EXIF, JPEG compressed)
